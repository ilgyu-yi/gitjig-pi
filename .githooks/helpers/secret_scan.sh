...
# scan. Its interface is stated by `.githooks/_lib.sh`. Sourced by
...
# agree on are the committed repo file, and the
...
#
# Measurement domain: the added text lines of the staged diff, per
...
# _project_ss_git_diff); added lines identified
...
# no-line-counts outcome, never a prose message. The first commit
...
#     SANITIZED path and never the matched bytes or raw path bytes.
#
...
# live at that surface. Every git child
# reads stdin from /dev/null (the adapter loop precedent). Fail-direction
# bookkeeping is stated here: a pattern match or unmeasurable staged input
# refuses, while unavailable scan machinery warns and allows.
#
# Env-trust note (the branch_guard.sh `unset -v` precedent): this file
# keeps no file-scope `_PROJECT_SS_*` state — every variable below is
# function-local, so there is no inherited cell for a caller's environment
# to pre-seed and nothing to discard at source time.

# _project_ss_sanitize_path <raw-path> — print the percent-encoded rendering
# (C-locale byte loop; printable ASCII except '%' passes through).
_project_ss_sanitize_path() {
	local LC_ALL=C
...
# _project_ss_disarm <reason-constant> — machinery degradation: exactly one
# not-enforced warn record AND one stderr line for the run, then the caller
# allows. Without the line this arm's allow is byte-identical on both
# streams to an enforced pass. The line makes that disarmed allow visibly
# different from an enforced pass. It stays one line for the run, not one per suppressed
...
# _project_ss_refuse_match <pattern-id> <raw-path> — pattern-match refusal:
# pattern ID + sanitized path on both surfaces, never the matched bytes.
_project_ss_refuse_match() {
	local _ss_id="$1" _ss_sp
	_ss_sp="$(_project_ss_sanitize_path "$2")"
...
# _project_ss_refuse_unmeasurable <raw-path> — the unmeasurable-input arm
#: its own cause, no pattern ID, content-free.
_project_ss_refuse_unmeasurable() {
	local _ss_sp
	_ss_sp="$(_project_ss_sanitize_path "$1")"
...
# _project_ss_git_diff <args…> — every staged-diff read goes through here:
...
#. Replace-object grafting is disabled on
...
# _project_ss_scan_path <raw-path> — scan one staged path's added lines.
...
_project_ss_scan_path() {
	local _ss_p="$1"
	local _ss_num _ss_first _ss_add _ss_rest _ss_del
	if ! _ss_num="$(_project_ss_git_diff --cached --no-ext-diff --no-textconv --no-color --numstat "$_ss_base" -- ":(literal)$_ss_p" 2>/dev/null </dev/null)"; then
		_project_ss_refuse_unmeasurable "$_ss_p"
		return 1
	fi
	if [ -z "$_ss_num" ]; then
		...
		# no change. Vouching here would approve unmeasured: any
		# interference between the two reads lands as a loud refusal.
		_project_ss_refuse_unmeasurable "$_ss_p"
		return 1
	fi
	# Binary is the numstat no-line-counts outcome: "-<TAB>-<TAB>...".
	_ss_first="${_ss_num%%$'\n'*}"
	_ss_add="${_ss_first%%$'\t'*}"
	_ss_rest="${_ss_first#*$'\t'}"
	_ss_del="${_ss_rest%%$'\t'*}"
	...
	local _ss_diff
	if ! _ss_diff="$(_project_ss_git_diff --cached --no-ext-diff --no-textconv --no-color -U0 "$_ss_base" -- ":(literal)$_ss_p" 2>/dev/null </dev/null)"; then
		_project_ss_refuse_unmeasurable "$_ss_p"
		return 1
	fi
	...
	while IFS= read -r _ss_dline; do
		...
		*) continue ;;
		...
		while [ "$_ss_i" -lt "$_ss_n" ]; do
			...
			_ss_rc=$?
			if [ "$_ss_rc" -eq 0 ]; then
				_project_ss_refuse_match "${_ss_ids[$_ss_i]}" "$_ss_p"
				return 1
			elif [ "$_ss_rc" -ne 1 ]; then
				# A matcher failure at scan time over one input is the
				# input side of the timing split, never a disarm.
				_project_ss_refuse_unmeasurable "$_ss_p"
				return 1
			fi
			_ss_i=$((_ss_i + 1))
...
scan_staged_secrets() {
	# LC_ALL=C for the run: [[ =~ ]] matches under the pinned byte semantics.
	# Function-local, restored on return.
	local LC_ALL=C
	local _ss_top _ss_pf _ss_home
	_ss_top="$(git rev-parse --show-toplevel 2>/dev/null </dev/null)" || _ss_top=""
	if [ -z "$_ss_top" ]; then
		_project_ss_disarm 'repository toplevel unresolvable'
		return 0
	fi

	# Up-front pattern validation applies to the whole run: any invalid row
	# disarms before any path
	...
	if [ ! -f "$_ss_pf" ] || [ ! -r "$_ss_pf" ]; then
		_project_ss_disarm 'pattern file absent or unreadable'
		return 0
	fi
	...
	while IFS= read -r _ss_row || [ -n "$_ss_row" ]; do
		...
		# added line — an armed-looking scan that checks nothing.
		_ss_row="${_ss_row%$'\r'}"
		case "$_ss_row" in
		'#'*) continue ;;
		esac
	...
	if [ "$_ss_bad" -ne 0 ]; then
		_project_ss_disarm 'pattern file failed up-front validation'
		return 0
	fi
	if [ "$_ss_n" -eq 0 ]; then
		_project_ss_disarm 'pattern set empty after stripping comments and blanks'
		return 0
	fi

	# Diff base, keyed by outcome: a born HEAD diffs against HEAD,
	...
	if git rev-parse -q --verify 'HEAD^{commit}' >/dev/null 2>&1 </dev/null; then
		...
		if [ -z "$_ss_base" ]; then
			_project_ss_disarm 'staged-diff base unresolvable'
			return 0
		fi
	fi
	...
	# streaming an empty list and allowing with zero records. Spool home is the
	# repository's own git dir first (writable at commit time), then the ambient
	# temp dir as fallback. An inherited TMPDIR must not become a silent disarm
	# lever, so both paths are measured. The fallback is one of the shell's
	...
	if [ -n "$_ss_gd" ] && [ -d "$_ss_gd" ]; then
		_ss_list="$(mktemp "$_ss_gd/project-secret-scan.XXXXXX" 2>/dev/null </dev/null)" || _ss_list=""
	fi
	if [ -z "$_ss_list" ]; then
		_ss_list="$(mktemp "${TMPDIR:-/tmp}/project-secret-scan.XXXXXX" 2>/dev/null </dev/null)" || _ss_list=""
	fi
	if [ -z "$_ss_list" ]; then
		_project_ss_disarm 'staged-path enumeration spool unavailable'
		return 0
	fi
	if ! _project_ss_git_diff --cached --name-only -z "$_ss_base" -- >"$_ss_list" 2>/dev/null </dev/null; then
		rm -f -- "$_ss_list"
		_project_ss_disarm 'staged path enumeration failed'
		return 0
	fi

	# Allow-list: domain exclusion, never approval.
	...
	while IFS= read -r -d '' _ss_path; do
		...
		if [ "$_ss_skip" -eq 0 ]; then
			if ! _project_ss_scan_path "$_ss_path"; then
				_ss_verdict=1
				break
			fi
		fi
