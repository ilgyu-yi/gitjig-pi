...
Issue completion is PR-bound except for the two routes SPEC §2.2 defines: Directive completion review, and — for an activated non-Directive Issue whose governed-repository change surface is expressly empty — empty-change completion review on that section's terms. Neither substitutes for a PR where repository implementation exists.
...
Inspect `before`, every ordered `steps[*].{method,path,body,postRead}`, optimistic ids/instants, `beforeDigest`, `desiredDigest`, the derived `correlationId`, the planner-owned whole `artifactHash`, and reverse-order `rollback` entries. In a later activated phase only, apply exactly an authorized artifact one step at a time with `gh api --method "$method" "$path" --input "$body_file"`, perform its declared GET post-read after each step, and stop on any mismatch. Recovery likewise uses only that artifact's rollback bodies followed by fresh GETs; never improvise a direct push or broader bypass.

Expected end state is one default-branch `core-governance` ruleset with no bypass and one `human-approval` ruleset whose only bypass is repository role 5 in pull-request mode. Repository merge settings allow merge commits and disable squash/rebase. Until Phase 4B independently proves that bypassing the quorum ruleset leaves the core ruleset enforced, every generated plan remains blocked. Partial server state is recovered by re-reading all inputs and producing a new plan; never replay stale calls or guess rollback values.

The plan remains unauthorized data. A later authorization is valid only when one completely paginated GET-only read finds one unedited platform comment, its author is the current authenticated actor with freshly read `admin` collaborator permission, and its explicit issued/expiry window, repository, live pair, correlation id and whole artifact hash all match. Pre-split source and post-split carrier plans necessarily have different hashes and records; neither transfers. Phase 4B-0 creates no authorization record and exposes no executor.
