...
Each repository's selections live at `.github/gitjig-governance.json`; every supported capability is explicitly selected, disabled, or unmanaged/preserve-only. The pure shared parser, validator, planner, hasher, and auditor is `.github/workflows/gitjig-governance.mjs`. Later operator surfaces derive over that engine: the human CLI at `.github/bin/gitjig-governance.mjs` with `configure`, `plan`, `apply`, and `audit`, and the Pi surface without reimplementation. Interactive human, agent-assisted, and non-interactive operation share config and semantics. Config changes and pure plans never mutate or authorize server state: later apply requires a complete GET-only plan, displayed exact repository and plan hash, explicit confirmation, per-write post-read, and final audit. Partial or ambiguous writes stop without invented success or automatic rollback.
