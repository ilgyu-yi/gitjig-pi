...
/** Bounded JSON-stdin mutation transport; an absent acknowledgement is an unverifiable outcome, never success. */
export function runPlatformJsonMutation(argv: string[], body: unknown, repoRoot: string): Promise<string | undefined> {
	let input: string;
	try {
	...
	return new Promise((resolve) => {
		...
		child.once("close", (code) => finish(!terminating && code === 0 ? output + decoder.end() : undefined));
