...
function assertBindSucceeded(run: InstrumentRun, arm: string): void {
	assert.equal(
		run.status,
		0,
		`${arm}: the bind run did not report a verified bound state; ` + `status=${run.status}\n${run.output}`,
	);
}
...
describe(
	"the exclusion fallback refuses to write through a link (issue #68 AC4, SPEC §5.5)",
	{ skip: IS_WINDOWS },
	() => {
...
describe(
	"the local hooksPath read keeps empty apart from absent (issue #68 AC3, SPEC §4.7)",
	{ skip: IS_WINDOWS },
	() => {
...
describe(
	"a ~/-spelled hooksPath is compared as git resolves it (issue #68 AC2, SPEC §4.7)",
	{ skip: IS_WINDOWS },
	() => {
	...
	it("concurrent bash and Node appends interleave by whole lines only, at a record size inside the single-append bound (mixed-writer sink bound)", async () => {
		...
		try {
			...
			for (let i = 0; i < 40; i += 1) {
				nodeOk =
					appendAuditRecord(stateRoot, { category: "secret", action: "warn", text: `zqnodewriter ${i}` }) && nodeOk;
			}
			await Promise.all(
				children.map(
					(child) =>
			...
			const lines = readFileSync(sink, "utf8")
				.split("\n")
				.filter((line) => line !== "");
			assert.equal(
				lines.length,
...
describe(
	"the derived sink stays writable and refuses an object it did not create (issue #68, SPEC §4.6, §5.5)",
	{ skip: IS_WINDOWS },
	() => {
...
describe(
	"a hostile git environment cannot retarget the bind (issue #68, #39 constructed environment)",
	{ skip: IS_WINDOWS },
	() => {
...
describe(
	"the arming verdict is refused where the adapters would not run (issue #68, SPEC §3.2, §5.2)",
	{ skip: IS_WINDOWS },
	() => {
		...
		it("a foreign hooks path reaches the terminal with its control bytes removed (issue #47)", () => {
			...
			try {
				...
				assert.notEqual(
					run.status,
					0,
					`hostile hooksPath: the run did not refuse, so no message is measured\n${run.output}`,
				);
				assert.equal(
...
describe(
	"the exclusion re-ask's recovery terminates on every shape reaching it (issue #74, SPEC §3.11)",
	{ skip: IS_WINDOWS },
	() => {
		...
		function performOneStep(root: string): "tracked" | "named" | "bounded" | "no-step" {
			...
			const isTracked = (): boolean => git(root, ["ls-files", "--error-unmatch", "--", SINK_POSIX]).status === 0;
			if (isTracked()) {
		 ...
		 */
		const EXTRAS: ReadonlyArray<string | null> = [null, "*.jsonl", "/.gitjig/state/audit.jsonl", ".gitjig", "/.git*"];
