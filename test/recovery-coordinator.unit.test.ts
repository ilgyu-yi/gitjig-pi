...
describe("Phase-A history recovery coordinator", () => {
	...
	it("refuses a second recovery request at the consumed-allowance guard", async () => {
		const current = {
			...subject,
			context: { ...subject.context, pullRequest: { ...subject.context.pullRequest, id: "PR_CONSUMED_COORDINATOR" } },
		};
		let dispatchCount = 0;
		const input = {
			repoRoot: process.cwd(),
			...
			history,
			basis,
			diagnosis: { value: "STAGNATION", invalidation: "nothing", evidence: "history evidence" } as const,
			...
			dispatchProfile: async () => {
				dispatchCount += 1;
				throw new Error("first claim consumed without an admitted return");
			},
		};
		const first = await coordinateHistoryRecovery(input);
		assert.equal(first.terminal, "handoff");
		assert.ok(dispatchCount > 0);
