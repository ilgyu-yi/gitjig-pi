...
describe("issue #238 repair-basis projection", () => {
	...
	it("withholds the whole basis on absent attestation or empty/unknown attribution", async () => {
		const root = repo();
		const a = commit(root, "a", "first");
		const b = commit(root, "a", "second");
	...
	it("parser rejects malformed provenance before history can consume it", () => {
		const input = record("a".repeat(40), [
			{ finding: "raw", validity: "CONFIRMED", severity: "SUBSTANTIVE", disposition: "repair" },
		]);
		for (const malformed of [null, { lens: "runtime", surface: 42 }]) {
			const copy = structuredClone(input);
			if (copy.adjudication) copy.adjudication.rulings[0].provenance = [malformed as never];
			assert.equal(parseReviewRecord(composeReviewRecord(copy)), undefined);
		}
	});
