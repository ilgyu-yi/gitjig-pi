...
describe("state-domain allowance store", () => {
	it("claims once, persists complete bytes, and atomically terminalizes without reopening", () => {
		...
		assert.deepEqual(JSON.parse(readFileSync(path, "utf8")).changeKey, encoding(current).operands);
...
`;
	...
	it("keeps a finalized allowance consumed after mode, head, branch and replan changes", () => {
		...
		assert.deepEqual(durable.changeKey, path.operands);
	...
	it("refuses mismatched change-key operands before claim and binds them through terminalization", () => {
		const current = subject("PR_KEY_OPERANDS");
		const mismatched = record(current);
		mismatched.changeKey = { repositoryNodeId: "OTHER_REPOSITORY", subject: mismatched.changeKey.subject };
		assert.deepEqual(claimAllowance({ subject: current, record: mismatched }), {
		...
		const terminal = consumed(claimed);
		terminal.changeKey = { repositoryNodeId: "OTHER_REPOSITORY", subject: terminal.changeKey.subject };
		assert.equal(finalizeAllowance(result.claim, terminal).status, "consumed-unverified");
	});
